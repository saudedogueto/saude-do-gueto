...
export default function RelatoriosScreen() {
  ...
  const exportarDados = async () => {
    try {
      ...
      await Share.share({ message: csv, title: 'Relatorio Saude do Gueto - Completo' });
    } catch {
  ...
  const exportarVisitas = async () => {
    try {
      ...
      const csv = cabecalhoGeral + '\n' + linhasVisitas;
      await Share.share({ message: csv, title: 'Relatorio de Visitas - Saude do Gueto - Completo' });
    } catch (error) {
  ...
  const exportarRelatorioCompleto = async () => {
    try {
      ...
      const csv = cabecalho + '\n' + linhasP + secVisitas + '\n' + linhasV;
      await Share.share({ message: csv, title: 'Relatorio Completo - Saude do Gueto' });
    } catch (error) {
      Alert.alert('Erro', 'Nao foi possivel exportar os dados');
    }
  };
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: cores.fundo }]}>
      ...
      <Text style={styles.exportInfo}>
        CSV com resumo + dados - compativel com Excel, Google Sheets e e-SUS
      </Text>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 24, fontWeight: 'bold', color: '#00E676', textAlign: 'center', marginTop: 20, marginBottom: 20 },
  abaRow: { flexDirection: 'row', gap: 8, marginBottom: 16 },
  abaBtn: { flex: 1, paddingVertical: 12, backgroundColor: '#00E676', borderRadius: 10, alignItems: 'center' },
  ...
  periodoText: { fontSize: 13, color: 'rgba(255,255,255,0.6)', fontWeight: '600' },
  periodoTextAtivo: { color: '#0B1220' },
  cardTotal: { backgroundColor: '#00E676', borderRadius: 16, padding: 25, alignItems: 'center', marginBottom: 20, elevation: 4, shadowColor: '#00E676', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 },
  cardTotalNumero: { fontSize: 42, fontWeight: 'bold', color: '#0B1220' },
  cardTotalLabel: { fontSize: 15, color: 'rgba(0,0,0,0.8)', marginTop: 5 },
  sectionTitle: { fontSize: 17, fontWeight: 'bold', color: '#FFFFFF', marginBottom: 12, marginTop: 5 },
  subSectionTitle: { fontSize: 14, fontWeight: '600', color: 'rgba(255,255,255,0.6)', marginBottom: 5 },
  grid: { flexDirection: 'row', gap: 10, marginBottom: 20 },
  card: { flex: 1, borderRadius: 12, padding: 12, alignItems: 'center' },
  cardNumero: { fontSize: 26, fontWeight: 'bold', color: '#FFFFFF' },
  cardLabel: { fontSize: 11, color: 'rgba(255,255,255,0.6)', marginTop: 2 },
  cardPct: { fontSize: 11, color: 'rgba(255,255,255,0.4)', marginTop: 2 },
  visitasCard: { backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: 16, marginBottom: 25, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
  visitaRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6 },
  visitaLabel: { fontSize: 14, color: 'rgba(255,255,255,0.6)' },
  visitaValor: { fontSize: 14, fontWeight: 'bold', color: '#FFFFFF' },
  divisor: { height: 1, backgroundColor: 'rgba(255,255,255,0.1)', marginVertical: 8 },
  exportRow: { flexDirection: 'row', gap: 12, marginBottom: 10 },
  exportBtn: { flex: 1, backgroundColor: 'rgba(255,255,255,0.06)', borderWidth: 2, borderColor: '#00E676', borderRadius: 10, paddingVertical: 14, alignItems: 'center' },
  exportText: { color: '#00E676', fontWeight: 'bold', fontSize: 14 },
  exportBtnCompleto: { backgroundColor: '#00E676', borderRadius: 10, paddingVertical: 16, alignItems: 'center', marginBottom: 10, elevation: 3, shadowColor: '#00E676', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 5 },
  exportTextCompleto: { color: '#0B1220', fontWeight: 'bold', fontSize: 15 },
  exportInfo: { fontSize: 12, color: 'rgba(255,255,255,0.4)', textAlign: 'center', marginBottom: 10 },
});
